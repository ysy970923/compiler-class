 ...
 *  returns: void
 */
static void insert(id *id, decl *decl, bool isglobal)
{
    ...
    else
    {
        if (stacktop == 1)
        {
            if (scopestack[stacktop] == scopestack[stacktop - 1])
            {
 ...
 */
static bool check_isvar(decl *decl)
{
 ...
 */
static bool check_isstruct(decl *typedecl)
{
 ...
 */
static bool check_isptr(decl *typedecl)
{
...
decl *makearraydecl(decl *num_index, decl *elementvar)
{
    ...
    declnode->num_index = num_index->intval;
    return declnode;
}
...
decl *address_op(decl *vardecl)
{
    ...
    declnode->declclass = VAR;
    declnode->typeclass = type;
    declnode->type = makeptrdecl(vardecl->type);
    return declnode;
}
...
decl *functioncall(decl *procdecl, decl *actuals)
{
    ...
    return makeconstdecl(procdecl->returntype);
}
...
decl *functionreturn(decl *returndecl)
{
    ...
    while (stenode)
    {
        if (check_isfunction(stenode->decl))
        {
            if (!check_sametype(stenode->decl->returntype, returndecl->type))
            {
